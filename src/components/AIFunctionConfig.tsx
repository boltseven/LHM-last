...
interface AIFunctionConfigProps {
  ...
  onChange: (config: AIFunctionConfig) => void;
  onGenerate: () => void;
  isLoading: boolean;
  disabled: boolean;
}
...
export const AIFunctionConfig: React.FC<AIFunctionConfigProps> = ({
  functionName,
  config,
  onChange,
  onGenerate,
  ...
  const handleConfigChange = (key: keyof AIFunctionConfig, value: any) => {
    onChange({
      ...config,
      [key]: value,
    });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="font-medium">{functionName} Configuration</h4>
        </div>
        <button
          onClick={onGenerate}
